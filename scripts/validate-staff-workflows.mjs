...
function seededAuthState() {
  return {
    ...
    sessions: [{
      id: "sess_rohit_staff",
      userId: "stf_demo_1",
      refreshToken: "refresh_rohit_staff",
      device: "Validation browser",
      ip: "127.0.0.1",
      expiresAt: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString(),
    }],
    auditLogs: [],
    currentUserId: "stf_demo_1",
    accessToken: "jwt_rohit_staff",
  };
}
...
for (const tab of tabs) {
  ...
  const url = `${baseUrl}/dashboard/staff/${tab}`;
  try {
    ...
    const hasStaffContext = bodyText.includes("Rohit Sharma") || bodyText.includes("Staff");
    const expectedTabContentVisible = bodyText.length > 100;

    const failed = !response ||
      response.status() >= 400 ||
      Boolean(fatalText) ||
      browserErrors.length > 0 ||
      !currentUrl.includes(`/dashboard/staff/${tab}`) ||
      !hasStaffContext ||
...
console.log("\nStaff workflow validation summary");
